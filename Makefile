...
# Исходные файлы
SOURCES := $(shell find . -name "*.cpp" ! -path "./build/*" ! -path "./.vscode/*") # Ищем .cpp во всех подпапках проекта кроме ./build/ и ./.vscode/

# Пути к заголовкам 
INCLUDES := $(shell find . -type d ! -path "./build/*" ! -path "./.vscode/*") # Ищем .h во всех подпапках проекта кроме ./build/ и ./.vscode/
